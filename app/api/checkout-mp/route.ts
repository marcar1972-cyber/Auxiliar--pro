...
// Inicialización de Firebase Admin unificada, limpia e indestructible para producción
...
async function procesarPago(request: Request) {
  console.log("🔔 === PROCESANDO FLUJO DE COBRO EN CHECKOUT-MP ===");
  ...
  const REDIRECT_DASHBOARD = "https://www.auxiliaresdefarmacia.cl/dashboard";

  if (!id) {
    console.log("⚠️ No se detectó ID de transacción en la petición. Redirigiendo a Dashboard.");
  ...
  try {
    ...
    if (!response.ok) {
      console.error("❌ Falla al validar el pago en la API de Mercado Pago.");
      return NextResponse.redirect(REDIRECT_DASHBOARD);
    }
    ...
    if (status !== "approved") {
      console.log(`⏳ Pago ID ${id} no approved (${status}). Redirigiendo.`);
      return NextResponse.redirect(REDIRECT_DASHBOARD);
    }
    ...
    if (logDoc.exists) {
      console.log(`⚠️ MP-Gateway: ID ${id} ya fue procesado con anterioridad.`);
      return NextResponse.redirect(REDIRECT_DASHBOARD);
    }
    ...
    console.log(`🎉 Transacción exitosa procesada en retorno para el alumno: ${uid}`);
    return NextResponse.redirect(REDIRECT_DASHBOARD);

  } catch (error: any) {
    console.error("❌ Error ejecutando callback de Checkout:", error.message);
    return NextResponse.redirect(REDIRECT_DASHBOARD);
  }
}
...
export default function QuizPage() {
  ...
  // VISTA RESULTADOS CON RETROALIMENTACIÓN Y LINK A BIBLIOTECA
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 text-left font-sans">
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl text-center max-w-lg w-full border border-white">
                ...
                {mistakes.map((m, i) => (
                    <div key={i} className="mb-6 last:mb-0 border-b border-slate-200 pb-4 last:border-0">
                        ...
                        <div className="space-y-1">
                          <p className="text-xs text-red-500 font-bold italic">Tu respuesta: {m.yourAnswer}</p>
                          <p className="text-xs text-emerald-600 font-bold italic">Correcta: {m.correctAnswer}</p>
                          <div className="bg-white p-3 rounded-lg border border-slate-100 mt-3">
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter mb-1">Debes reforzar:</p>
                             <p className="text-xs text-slate-500 italic leading-relaxed">{m.studyGuide}</p>
                          </div>
                        </div>
                    </div>
                ))}
          ...
          <Link href="/biblioteca" className="flex items-center justify-between bg-emerald-50 p-5 rounded-2xl mb-8 border border-emerald-100 group hover:bg-emerald-500 transition-colors cursor-pointer">
              <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                      <p className="text-[10px] font-black text-emerald-600 group-hover:text-white uppercase">Refuerza en la</p>
                      <p className="text-sm font-bold text-slate-800 group-hover:text-white">Biblioteca Técnica</p>
                  </div>
              </div>
              ...
              <button key={l.id} onClick={() => startLevel(l.id)} disabled={!isU} className={`w-full group rounded-[2rem] border-2 transition-all p-6 flex items-center gap-6 text-left ${isP ? "bg-emerald-50 border-emerald-100 cursor-pointer" : isU ? "bg-white border-white shadow-xl hover:-translate-y-1 cursor-pointer" : "bg-slate-100 opacity-60 grayscale cursor-default"}`}>
                ...
                <div className="flex-1 text-left">
                    ...
                    <div className="flex items-center gap-3 mt-0.5">
                        <p className="text-xs font-bold text-slate-400">{l.questions.length} Preguntas</p>

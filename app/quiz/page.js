...
export default function QuizPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 text-left">
        <div className="w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 text-left">
          <div className="w-full bg-slate-100 h-3 text-left"><div className="bg-emerald-500 h-3 transition-all duration-500" style={{ width: `${((currentQIndex + 1) / level.questions.length) * 100}%` }}></div></div>
          <div className="p-8 md:p-12 text-left">
            <div className="flex justify-between items-center mb-10 text-left"><span className="text-xs font-black text-slate-400 uppercase text-left">Pregunta {currentQIndex + 1} de {level.questions.length}</span>{timeLeft > 0 ? <div className="bg-slate-100 px-4 py-2 rounded-full font-mono font-bold text-slate-600 flex items-center gap-2"><Clock size={16}/> {Math.floor(timeLeft/60)}:{timeLeft%60<10?'0':''}{timeLeft%60}</div> : <div className="bg-emerald-50 px-4 py-2 rounded-full font-bold text-emerald-600 text-[10px] uppercase tracking-wider">Modo Práctica</div>}</div>
  ...
  if (showResult && activeLevelId) {
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 text-left">
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl text-center max-w-lg w-full text-left border border-white">
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-24 font-sans text-left">
      <div className="bg-white p-6 shadow-sm sticky top-0 z-10 flex items-center justify-between text-left">
        <div className="flex items-center gap-4 text-left"><Link href="/" className="text-slate-400 hover:text-slate-900 cursor-pointer text-left"><ChevronLeft size={28} /></Link><h1 className="text-xl font-black text-slate-900 tracking-tighter text-left">Mi Ruta 2026</h1></div>
        <div className="bg-slate-100 px-4 py-2 rounded-full text-sm font-black text-slate-700 flex items-center gap-3 text-left"><Trophy size={18} className="text-yellow-500"/> {unlockedLevels.length - 1} / 4</div>
      </div>
      ...
      <div className="p-6 max-w-xl mx-auto space-y-8 mt-6 text-left">
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex items-center gap-5 text-left">
          <div className="relative text-left">{user?.photoURL ? (<img src={user.photoURL} alt="P" className="w-16 h-16 rounded-2xl border-4 border-slate-50 shadow-md object-cover" />) : (<div className="w-16 h-16 bg-emerald-500 text-white rounded-2xl flex items-center justify-center text-2xl font-black shadow-lg">M</div>)}<div className="absolute -bottom-1 -right-1 bg-blue-500 text-white p-1 rounded-lg border-2 border-white text-left"><ShieldCheck size={14}/></div></div>
              ...
              <button key={l.id} onClick={() => startLevel(l.id)} disabled={!isU} className={`w-full group rounded-[2rem] border-2 transition-all p-6 flex items-center gap-6 text-left ${isP ? "bg-emerald-50 border-emerald-100 cursor-pointer" : isU ? "bg-white border-white shadow-xl hover:-translate-y-1 cursor-pointer" : "bg-slate-100 opacity-60 grayscale cursor-default"}`}>
                ...
                <div className="flex-1 text-left">
                    ...
                    <div className="flex items-center gap-3 mt-0.5 text-left text-xs">
                        <p className="font-bold text-slate-400 text-left">{l.questions.length} Preguntas</p>
        ...
        {/* --- NUEVAS TARJETAS MODULARES EN EL QUIZ --- */}
        <div className="grid grid-cols-1 gap-4 text-left mt-10">
            <Link href="/biblioteca" className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 shadow-sm hover:border-blue-400 transition-all flex items-center gap-6 group text-left cursor-pointer">
                <div className="w-14 h-14 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform text-left"><Library size={28} /></div>
                <div className="text-left"><h3 className="font-black text-lg text-slate-800 tracking-tight text-left text-left">Biblioteca</h3><p className="text-xs text-slate-400 font-bold uppercase text-left italic">Recursos PDF</p></div>
            </Link>

            <Link href="/foro" className="bg-[#0f172a] p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group hover:scale-[1.01] transition-all cursor-pointer border border-white/10 text-left">
                <div className="flex items-center justify-between relative z-10 text-left">
                    <div className="text-left text-left pr-4 text-left">
                        <h3 className="text-xl font-black text-white italic text-left">¿Dudas Técnicas?</h3>
                        <p className="text-slate-400 text-xs mt-1 text-left">Entra al Foro Pro</p>
                    </div>
                    <div className="shrink-0 text-left text-left"><WhatsAppIcon size={32} className="text-emerald-500 group-hover:text-white transition-colors text-left" /></div>
                </div>
            </Link>
        </div>
      </div>

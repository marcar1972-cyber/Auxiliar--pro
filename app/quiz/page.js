...
import { 
  ...
  ThermometerSnowflake, Facebook, Instagram, ArrowRight
} from "lucide-react"; 
...
export default function QuizPage() {
  ...
  // --- VISTA EXAMEN ACTIVO (CON BOTÓN DE SALIDA RESTAURADO) ---
  if (activeLevelId && !showResult) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 text-left">
        ...
        {/* BOTÓN RESTAURADO AQUÍ */}
        <button 
          onClick={returnToMenu} 
          className="mt-8 text-slate-400 font-bold hover:text-red-500 transition-colors cursor-pointer"
        >
          Abandonar Examen
        </button>
      </div>
        ...
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl text-center max-w-lg w-full border border-white">
          ...
          {mistakes.length > 0 && (
            <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 text-left mb-8 max-h-60 overflow-y-auto text-left">
                ...
                {mistakes.map((m, i) => (
                    <div key={i} className="mb-6 last:mb-0 border-b border-slate-200 pb-4 last:border-0 text-left">
                        <p className="font-bold text-slate-800 mb-2 text-sm leading-snug text-left">{m.question}</p>
                        <div className="space-y-1 text-left">
                          <p className="text-xs text-red-500 font-bold italic text-left">Tu respuesta: {m.yourAnswer}</p>
                          <p className="text-xs text-emerald-600 font-bold italic text-left">Correcta: {m.correctAnswer}</p>
                          <div className="bg-white p-3 rounded-lg border border-slate-100 mt-3 text-left">
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter mb-1 text-left">Debes reforzar:</p>
                             <p className="text-xs text-slate-500 italic leading-relaxed text-left">{m.studyGuide}</p>
                          </div>
                        </div>
                    </div>
                ))}
          ...
          <Link href="/biblioteca" className="flex items-center justify-between bg-emerald-50 p-5 rounded-2xl mb-8 border border-emerald-100 group hover:bg-emerald-500 transition-colors cursor-pointer text-left">
              <div className="flex items-center gap-3 text-left">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-emerald-600"><Library size={20}/></div>
                  <div className="text-left">
                      <p className="text-[10px] font-black text-emerald-600 group-hover:text-white uppercase text-left">Refuerza en la</p>
                      <p className="text-sm font-bold text-slate-800 group-hover:text-white text-left">Biblioteca Técnica</p>
                  </div>
              </div>
              <ArrowRight size={20} className="text-emerald-300 group-hover:text-white"/>
          </Link>
    ...
    <main className="min-h-screen bg-slate-50 pb-24 font-sans text-left">
            ...
            return (
              <button key={l.id} onClick={() => startLevel(l.id)} disabled={!isU} className={`w-full group rounded-[2rem] border-2 transition-all p-6 flex items-center gap-6 text-left ${isP ? "bg-emerald-50 border-emerald-100 cursor-pointer" : isU ? "bg-white border-white shadow-xl hover:-translate-y-1 cursor-pointer" : "bg-slate-100 opacity-60 grayscale cursor-default"}`}>
                ...
                <div className="flex-1 text-left">
                    ...
                    <div className="flex items-center gap-3 mt-0.5 text-left">
                        <p className="text-xs font-bold text-slate-400 text-left">{l.questions.length} Preguntas</p>
                        <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full border ${isP ? "bg-emerald-100 text-emerald-600 border-emerald-200" : isU ? "bg-slate-100 text-slate-400 border-slate-200" : "hidden"}`}>
                            {isP ? "Superado" : "Pendiente de evaluación"}
                        </span>
                    </div>
                </div>
              </button>
      ...
      {showRulesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-in fade-in zoom-in duration-200">
            <div className="bg-white rounded-[3rem] shadow-2xl max-sm:w-full max-w-sm w-full overflow-hidden border border-white text-left">

...
export default function QuizPage() {
  ...
  const handleAnswer = (optionIndex) => {
    ...
    setIsAnswered(true);
    if (optionIndex === question.correctIndex) { setScore((prev) => prev + 1); }
    else { setMistakes(prev => [...prev, { question: question.text, yourAnswer: question.options[optionIndex], correctAnswer: question.options[question.correctIndex], studyGuide: question.studyGuide }]); }
    setTimeout(() => {
    ...
    const saveProgress = async () => {
      if (showResult && activeLevelId && user) {
        ...
        try {
          ...
          await updateDoc(userRef, { quizHistory: arrayUnion({ levelId: activeLevelId, score: score, passed: passed, date: new Date().toISOString() }) });
          if (passed) {
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 text-left">
        <div className="w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100">
          ...
          <div className="p-8 md:p-12">
            <div className="flex justify-between items-center mb-10"><span className="text-xs font-black text-slate-400 uppercase tracking-widest">Pregunta {currentQIndex + 1} de {level.questions.length}</span>{timeLeft > 0 ? (<div className="bg-slate-100 px-4 py-2 rounded-full font-mono font-bold text-slate-600 flex items-center gap-2"><Clock size={16} className="text-emerald-500"/> {formatTime(timeLeft)}</div>) : (<div className="bg-emerald-50 px-4 py-2 rounded-full font-bold text-emerald-600 text-[10px] uppercase tracking-wider">Modo Práctica</div>)}</div>
            ...
            <div className="space-y-4">
              {question.options.map((opt, idx) => {
                ...
                if (isAnswered) { if (idx === question.correctIndex) style = "bg-green-50 border-green-500 text-green-700 shadow-sm"; else if (idx === selectedOption) style = "bg-red-50 border-red-500 text-red-700"; else style = "opacity-40 grayscale"; }
                return (<button key={idx} onClick={() => handleAnswer(idx)} disabled={isAnswered} className={`w-full text-left p-5 rounded-2xl border-2 font-bold transition-all flex items-center gap-4 ${style}`}><span className="w-8 h-8 rounded-lg bg-white border border-inherit flex items-center justify-center text-sm">{["A","B","C","D"][idx]}</span><span className="flex-1">{opt}</span></button>);
              })}
            </div>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 text-left font-sans">
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl text-center max-w-lg w-full border border-white">
          ...
          <div className="flex flex-col gap-3"><button onClick={passed ? returnToMenu : () => startLevel(activeLevelId)} className={`w-full font-black py-5 rounded-2xl shadow-lg transition-transform active:scale-95 ${passed ? "bg-slate-900 text-white" : "bg-emerald-500 text-white"}`}>{passed ? "Continuar Ruta" : "Reintentar Nivel"}</button><button onClick={returnToMenu} className="py-4 text-slate-400 font-bold text-sm text-center">Volver al Menú</button></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-24 font-sans text-left">
      <div className="bg-white p-6 shadow-sm sticky top-0 z-10 flex items-center justify-between">
        <div className="flex items-center gap-4"><Link href="/" className="text-slate-400 hover:text-slate-900 transition-colors"><ChevronLeft size={28} /></Link><h1 className="text-xl font-black text-slate-900 tracking-tighter">Mi Ruta 2026</h1></div>
        <div className="flex items-center gap-3 bg-slate-100 px-4 py-2 rounded-full"><Trophy size={18} className="text-yellow-500"/><span className="text-sm font-black text-slate-700">{unlockedLevels.length - 1} / 4</span></div>
      </div>
      ...
      <div className="p-6 max-w-xl mx-auto space-y-8 mt-6 text-left">
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex items-center gap-5 text-left">
          <div className="relative">{user?.photoURL ? (<img src={user.photoURL} alt="Profile" className="w-16 h-16 rounded-2xl border-4 border-slate-50 shadow-md object-cover" />) : (<div className="w-16 h-16 bg-emerald-500 text-white rounded-2xl flex items-center justify-center text-2xl font-black shadow-lg">{user?.displayName?.[0] || "M"}</div>)}<div className="absolute -bottom-1 -right-1 bg-blue-500 text-white p-1 rounded-lg border-2 border-white"><ShieldCheck size={14}/></div></div>
          <div className="text-left"><p className="text-[10px] text-emerald-600 font-black uppercase tracking-[0.2em] text-left">Auxiliar en formación</p><h2 className="text-xl font-black text-slate-900 leading-none mb-1 text-left">{user?.displayName || "Marcelo C"}</h2><p className="text-xs text-slate-400 font-medium italic text-left">Año de examen: 2026</p></div>
        </div>
        ...
        <div className="space-y-4">
          {LEVELS.map((level) => {
            ...
            return (
              <button key={level.id} onClick={() => startLevel(level.id)} disabled={!isUnlocked} className={`w-full group rounded-[2rem] border-2 transition-all p-6 flex items-center gap-6 text-left ${isPassed ? "bg-emerald-50 border-emerald-100" : isUnlocked ? "bg-white border-white shadow-xl hover:translate-y-[-4px]" : "bg-slate-100 border-transparent opacity-60 grayscale"}`}>
                <div className={`w-16 h-16 rounded-full flex items-center justify-center text-3xl shadow-inner transition-transform group-hover:scale-110 ${isPassed ? "bg-emerald-500 text-white" : isUnlocked ? "bg-[#dcfce7] text-[#10b981]" : "bg-slate-300 text-slate-400"}`}>{isPassed ? <CheckCircle size={32} /> : (isUnlocked ? (iconMap[level.icon] || level.icon) : <Lock size={28}/>)}</div>
                <div className="text-left"><h3 className="font-black text-lg text-slate-800 leading-tight text-left">{level.title}</h3><p className="text-xs font-bold text-slate-400 mt-0.5 text-left">{level.questions.length} Preguntas</p></div>
              </button>
            );
          })}
        </div>

        <div className="mt-8"><Link href="/biblioteca" className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 shadow-sm hover:border-blue-400 transition-all flex items-center gap-6 group text-left"><div className="w-14 h-14 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform"><Library size={28} /></div><div className="text-left"><h3 className="font-black text-lg text-slate-800 tracking-tight text-left">Biblioteca Técnica</h3><p className="text-xs text-slate-400 font-bold uppercase tracking-wider italic text-left">Descargar Material de Estudio PDF</p></div></Link></div>

        {/* --- TARJETA FORO CON RRSS --- */}
        <div className="mt-8 p-10 bg-[#0f172a] rounded-[3.5rem] shadow-2xl relative overflow-hidden text-center border border-white/5">
            ...
            <button onClick={() => setShowRulesModal(true)} className="bg-white text-slate-900 font-black py-5 px-10 rounded-3xl w-full flex items-center justify-center gap-3 hover:bg-slate-100 transition-all shadow-lg text-lg"><MessageCircle size={24} className="text-pink-500"/> Entrar al Foro</button>
        </div>
      </div>
      ...
      {showRulesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-in fade-in zoom-in duration-200">
            <div className="bg-white rounded-[3rem] shadow-2xl max-w-sm w-full overflow-hidden border border-white">
                ...
                <div className="p-8 space-y-6 text-left"><div className="flex gap-4 items-start"><div className="bg-emerald-100 p-2 rounded-lg text-emerald-600 shadow-sm"><ShieldCheck size={20} /></div><p className="text-sm text-slate-600 leading-snug text-left"><strong>Contenido Técnico:</strong> Prohibido spam y mensajes masivos. Solo info de farmacia.</p></div><div className="flex gap-4 items-start"><div className="bg-red-100 p-2 rounded-lg text-red-600 shadow-sm"><OctagonAlert size={20} /></div><p className="text-sm text-slate-600 leading-snug text-left"><strong>Respeto:</strong> Cualquier falta de respeto resultará en la <strong>eliminación inmediata</strong>.</p></div><button onClick={() => { window.open("https://chat.whatsapp.com/J4VkI8mzTTs9UrzvGqBbdz", "_blank"); setShowRulesModal(false); }} className="w-full bg-emerald-500 text-white font-black py-5 rounded-2xl hover:bg-emerald-400 transition-all flex items-center justify-center gap-3 mt-4 shadow-xl"><ThumbsUp size={20} /> Acepto e Ingresar</button><button onClick={() => setShowRulesModal(false)} className="w-full text-slate-400 text-sm font-bold py-2 text-center">Volver</button></div>
            </div>
        </div>
      )}
    </main>

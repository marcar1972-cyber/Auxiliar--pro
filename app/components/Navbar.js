...
import { 
  ...
  Star, Pill, HelpCircle, Share2, ShieldCheck, Trophy, ArrowRight
} from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <>
      {/* 🏆 BANNER DE CIERRE DE SORTEO & GANADORA */}
      <div className="bg-[#003366] text-white py-2 px-4 text-center z-[101] relative shadow-md">
        <Link href="/quiz" className="flex items-center justify-center gap-1 text-xs sm:text-sm font-bold hover:text-emerald-300 transition-colors group">
          <Trophy size={16} className="text-amber-400 shrink-0" />
          <span className="text-emerald-300">
            Sorteo Cerrado | Ganadora de la suscripción gratis mensual de Julio 2026: Samira 🎉
          </span>
          <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform ml-1 text-emerald-300" />
        </Link>
      </div>

  ...
  return (
    <main className="flex min-h-screen flex-col bg-white font-sans text-left">
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-md mx-auto w-full mt-12 mb-10">
        ...
        <h1 className="text-4xl md:text-5xl font-black text-aux-dark leading-[1.1] mb-8 tracking-tight text-center">
        ...
        <div className="w-full space-y-3">
            <Link href="/quiz" className="block w-full bg-aux-dark text-white font-bold text-lg py-4 rounded-xl shadow-lg shadow-blue-900/10 transition-all">
                COMENZAR AHORA
            </Link>
            <Link href="/blog" className="group block w-full bg-white text-slate-600 border-2 border-slate-100 font-bold text-lg py-4 rounded-xl hover:border-aux-green transition-all flex items-center justify-center gap-2">
        ...
        <div className="mt-12 bg-slate-50 p-5 rounded-2xl border border-slate-100 text-left w-full flex gap-4 items-start relative overflow-hidden">
            <div className="relative z-10 w-10 h-10 min-w-[40px] bg-white border border-slate-200 rounded-full flex items-center justify-center text-aux-green font-black shadow-sm">M</div>
        ...
        <div className="w-full mt-20 text-left">
            <div className="flex items-end justify-between mb-6 px-2 text-left">
                <h3 className="text-2xl font-black text-aux-dark tracking-tighter text-left">Centro de Ayuda</h3>
                <Link href="/faq" className="text-aux-green text-xs font-bold flex items-center gap-1 hover:underline">Ver todo <ArrowRight size={14} /></Link>
            </div>
            <div className="space-y-3 text-left">
                {featuredFaqs.map((faq, index) => (
                    <div key={index} className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm text-left">
                        <p className="text-sm font-bold text-slate-800 mb-2 flex gap-2 text-left"><HelpCircle size={16} className="text-emerald-500 shrink-0" />{faq.question}</p>
                        <p className="text-xs text-slate-500 leading-relaxed italic text-left">{faq.answer}</p>
                    </div>
                ))}
                <Link href="/faq" className="block bg-[#0f172a] p-6 rounded-[2rem] mt-6 relative overflow-hidden group transition-all text-left">
                    <div className="flex items-center justify-between relative z-10 text-left">
                        ...
                        <div className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center text-white"><ChevronDown size={20} className="-rotate-90" /></div>
                    </div>
                </Link>
            </div>
        </div>
      ...
      <footer className="w-full bg-slate-50 border-t border-slate-100 py-12 px-4 mt-auto">
        <div className="max-w-md mx-auto text-center space-y-8">
            <div className="flex justify-center gap-8 mb-4">
                <a href="https://www.facebook.com/profile.php?id=61584679565188" target="_blank" className="text-slate-400 hover:text-blue-600 transition-all"><Facebook size={24} /></a>
                <a href="https://www.instagram.com/auxiliarpro/" target="_blank" className="text-slate-400 hover:text-pink-500 transition-all"><Instagram size={24} /></a>
                <a href="https://chat.whatsapp.com/J4VkI8mzTTs9UrzvGqBbdz" target="_blank" className="text-slate-400 hover:text-emerald-500 transition-all"><WhatsAppIcon size={24} /></a>
            </div>
            ...
            <a href="mailto:auxiliaprofarma@gmail.com" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-tighter text-slate-400 bg-white px-4 py-2 rounded-full border border-slate-200 shadow-sm"><Mail size={12} /> auxiliaprofarma@gmail.com</a>
            <p className="text-[10px] text-slate-400 font-medium italic">© 2025 AuxiliarPro Chile. Proyecto independiente para el examen 2026.</p>
        </div>
      </footer>
    </main>
  );

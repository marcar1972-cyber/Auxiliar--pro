...
import { Montserrat, Inter } from "next/font/google"; // Optimización de fuentes
import { 
  Mail, HelpCircle, Facebook, 
  Instagram, Library, MessageCircle, Sparkles, ArrowRightCircle 
} from "lucide-react"; 
import { FAQS } from "./data"; 

// Configuración de fuentes para eliminar el renderizado tardío
  ...
  return (
    <main className={`${inter.variable} ${montserrat.variable} flex min-h-screen flex-col bg-white font-sans text-left`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-md mx-auto w-full mt-12 mb-10">
        ...
        {/* Optimización: logo.webp con prioridad para LCP */}
        ...
        <h1 className="text-4xl font-black text-aux-dark leading-[1.1] mb-8 tracking-tight text-center font-montserrat">
            AUXILIAR DE <br/>
        ...
        <div className="w-full space-y-4">
            <Link href="/quiz" aria-label="Comenzar ruta de evaluación técnica" className="block w-full bg-aux-dark text-white font-bold text-lg py-5 rounded-2xl shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer">
                RUTA DE EVALUACIÓN
            </Link>

            <div className="grid grid-cols-2 gap-3">
                <Link href="/biblioteca" aria-label="Ir a la biblioteca de recursos técnicos" className="bg-white text-slate-700 border-2 border-slate-100 font-bold py-4 rounded-2xl hover:border-blue-400 transition-all flex flex-col items-center gap-2 cursor-pointer shadow-sm">
                    <Library size={24} className="text-blue-500" aria-hidden="true" />
                    <span className="text-xs uppercase tracking-tighter">Biblioteca</span>
                </Link>
                <Link href="/foro" aria-label="Unirse a la comunidad de estudio" className="bg-white text-slate-700 border-2 border-slate-100 font-bold py-4 rounded-2xl hover:border-pink-400 transition-all flex flex-col items-center gap-2 cursor-pointer shadow-sm">
                    <MessageCircle size={24} className="text-pink-500" aria-hidden="true" />
                    <span className="text-xs uppercase tracking-tighter">Comunidad</span>
                </Link>
            </div>
        </div>

        {/* Optimización ARIA para accesibilidad en Fase 2 */}
        <Link href="/empleabilidad" aria-label="Ver tips de optimización de CV y empleabilidad" className="mt-8 block w-full bg-gradient-to-br from-slate-900 to-slate-800 p-8 rounded-[2.5rem] relative overflow-hidden group hover:scale-[1.01] transition-all cursor-pointer border border-white/10 shadow-2xl">
            ...
            <div className="flex items-center justify-between relative z-10">
                ...
                <div className="shrink-0">
                    <ArrowRightCircle size={48} className="text-emerald-500 group-hover:text-yellow-400 transition-colors" aria-hidden="true" />
                </div>
            </div>
        </Link>
        ...
        <div className="w-full mt-20 text-left">
            <h3 className="text-2xl font-black text-aux-dark tracking-tighter mb-6 px-2 font-montserrat">Dudas Frecuentes</h3>
      ...
      <footer className="w-full bg-slate-50 border-t border-slate-100 py-12 px-4 mt-auto">
        <div className="max-w-md mx-auto text-center space-y-8">
            <div className="flex justify-center gap-8 mb-4">
                <a href="https://www.facebook.com/profile.php?id=61584679565188" target="_blank" aria-label="Visita nuestro perfil de Facebook" className="text-slate-400 hover:text-blue-600 transition-all cursor-pointer"><Facebook size={24} aria-hidden="true" /></a>
                <a href="https://www.instagram.com/auxiliarpro/" target="_blank" aria-label="Síguenos en Instagram" className="text-slate-400 hover:text-pink-500 transition-all cursor-pointer"><Instagram size={24} aria-hidden="true" /></a>
                <a href="https://chat.whatsapp.com/J4VkI8mzTTs9UrzvGqBbdz" target="_blank" aria-label="Únete al grupo de apoyo en WhatsApp" className="text-slate-400 hover:text-emerald-500 transition-all cursor-pointer"><WhatsAppIcon size={24} aria-hidden="true" /></a>
            </div>
